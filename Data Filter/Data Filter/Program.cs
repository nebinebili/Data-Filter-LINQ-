...
namespace Data_Filter
{
    class Program
    {
        ...
        public static void Answer13()
        {
            ...
            {
                count = 0;
                for (int k = 0; k < debtors[i].Phone.Length; k++)
                {
                    for (int j = k + 1; j < debtors[i].Phone.Length; j++)
                    {
                        if ((debtors[i].Phone[k] > 47 && (debtors[i].Phone[k] < 58)) && (debtors[i].Phone[j] > 47 && (debtors[i].Phone[j] < 58)))
                        {

                            if (debtors[i].Phone[k] == debtors[i].Phone[j]) { ++count; }
                        }
                    }
                    if (count == 1) break;
                }
        ...
        public static bool Answer8(string str1, string str2)
        {
            int count = 0;
            int check = 0;
            int count1 = 0;
        ...
        static void Main()
        {
            ...
            //var list = debtors.FindAll(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
            //list.ForEach(d => Console.WriteLine(d));


            //2) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
            //var list = debtors.Where(d => ((DateTime.Now.Year - d.BirthDay.Year) > 26)&&((DateTime.Now.Year - d.BirthDay.Year) <36)).ToList();
            //list.ForEach(d => Console.WriteLine(d));


            // 3) Borcu 5000 - den cox olmayan borclularic cixartmag
            //var list = debtors.FindAll(d => d.Debt < 5000).ToList();
            //list.ForEach(d => Console.WriteLine(d));

            //4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
            //var list = debtors.FindAll(d => d.FullName.Length > 18 && d.Phone.Count(t => t == '7') >= 2).ToList();
            //list.ForEach(d => Console.WriteLine(d));

            // 5) Qishda anadan olan borclulari cixardmaq
            //var list = debtors.Where(d => (d.BirthDay.Month == 12) || (d.BirthDay.Month == 1) || (d.BirthDay.Month == 2)).ToList();
            //list.ForEach(d => Console.WriteLine(d));
            ...
            //var temp = debtors.Average(d => d.Debt);

            //surname filter
            //var list = debtors.FindAll(d => d.Debt > temp).OrderBy(d => d.FullName.Split('.')[1]).ToList();
            //list.ForEach(d => Console.WriteLine(d));

            //debt filter
            //var list = debtors.FindAll(d => d.Debt > temp).OrderByDescending(d => d.Debt).ToList();
            //list.ForEach(d => Console.WriteLine(d));


            // 7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
            //var list = debtors.FindAll(d => d.Phone.All(t => t != '8')).ToList();
            //list.ForEach(d =>
            //{
            //    Console.WriteLine($"Surname->{d.FullName.Split('.')[1]}");
            //    Console.WriteLine($"Age->{DateTime.Now.Year - d.BirthDay.Year}");
            //    Console.WriteLine($"Debt->{d.Debt}");
            //    Console.WriteLine("\n");
            //});
            ...
            //var list = debtors.FindAll(d => Answer8(d.FullName.Split('.')[0], d.FullName.Split('.')[1])).OrderBy(d => d.FullName).ToList();
            //list.ForEach(d => Console.WriteLine(d));
            ...
            // Console.WriteLine($"All Debt->{ debtors.Sum(d => d.Debt)}");


            // 12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
            //var list = debtors.Where(d => (d.BirthDay.Year < 1945)).ToList();
            //list.ForEach(d => Console.WriteLine(d));
            ...
            //var list = debtors.FindAll(d => d.FullName.ToLower().Count(t => t == 's') >= 1 
            //&& d.FullName.ToLower().Count(t => t == 'm') >= 1
            //&& d.FullName.ToLower().Count(t => t == 'i') >= 1 && 
            //d.FullName.ToLower().Count(t => t == 'l') >= 1 
            //&& d.FullName.ToLower().Count(t => t == 'e') >= 1).ToList();
        ...
        private static void Debtors(out List<Debtor> debtors)
        {
            ...
            new Debtor("Shirley T. Quallesss", DateTime.Parse("March 30, 1932"), "530-692-7177", "ShirleyTQualls@teleworm.us", "3565 Eagles Nest Drive Woodland, CA 95695", 2414),
            ...
            new Debtor("Bobb L. Zambranooo", DateTime.Parse("February 28, 1990"), "706-446-1649", "BobLZambrano@armyspy.com", "2031 Limer Street Augusta, GA 30901", 6628),
            ...
            new Debtor("Richard M. Thomas", DateTime.Parse("February 13, 1972"), "518-732-649", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 7875),
